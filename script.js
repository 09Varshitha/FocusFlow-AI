...
// IMPORTANT: Correct API base for Vercel
const BASE_URL = window.location.origin + "/api";
...
async function login() {
    ...
    try {
        let data = await safeFetch(`${BASE_URL}/login`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ email, password })
        });
...
async function register() {
    ...
    try {
        let data = await safeFetch(`${BASE_URL}/register`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ email, password })
        });
...
async function generate() {
    ...
    try {
        await safeFetch(`${BASE_URL}/generate`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
                subjects,
...
async function load() {
    ...
    try {
        let data = await safeFetch(
            `${BASE_URL}/data?user=${currentUser}`
        );
...
async function update(di, ti, field, val) {
    ...
    try {
        ...
        schedule[di].tasks[ti][field] =
            Number(val) || 0;

        await safeFetch(`${BASE_URL}/save`, {
            method: "POST",
            headers: {
